...
const FORMATOS_VALIDOS = [
    'Vertical 9:16 - 1080x1920px (Tik Tok / Reels / Shorts)',
    'Horizontal 16:9 - 1920x1080px HD (YouTube / cine)',
    'Horizontal 16:9 - 1280x720px HD (YouTube / cine)',
    'Cuadrado 1:1 - 1080x1080px  (Feed Instagram / Anuncios)'
];
...
function validarProducto(datos) {
    ...
    const regexFormato = /^(Vertical|Horizontal|Cuadrado)\s\d+:\d+\s-\s\d+x\d+px\s*(HD)?\s*\(.*\)$/i;

    if (!datos.formato || !regexFormato.test(datos.formato.trim())) {
        errores.push('El formato de video no cumple con la estructura permitida');
    }
...
router.get('/', async (req, res) => {
    try {
        const [servicios] = await db.execute(
            'SELECT id, nombre, precio, formato, created_at, updated_at FROM productos ORDER BY id ASC'
        );
        res.json({ status: 'success', data: servicios, count: servicios.length });
    } catch (error) {
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [servicios] = await db.execute(
            'SELECT id, nombre, precio, formato, created_at, updated_at FROM productos WHERE id = ?',
            [id]
        );
        if (servicios.length === 0) {
            return res.status(404).json({ status: 'error', message: `Servicio con ID ${id} no encontrado` });
        }
...
router.post('/', async (req, res) => {
    try {
        ...
        const { nombre, precio, formato } = req.body;
        const [resultado] = await db.execute(
            'INSERT INTO productos (nombre, precio, formato) VALUES (?, ?, ?)',
            [nombre.trim(), parseFloat(precio), formato]
        );
        const [nuevo] = await db.execute(
            'SELECT id, nombre, precio, formato, created_at FROM productos WHERE id = ?',
            [resultado.insertId]
        );
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const { nombre, precio, formato } = req.body;
        await db.execute(
            'UPDATE productos SET nombre = ?, precio = ?, formato = ? WHERE id = ?',
            [nombre.trim(), parseFloat(precio), formato, id]
        );
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [servicio] = await db.execute('SELECT id, nombre FROM productos WHERE id = ?', [id]);
        ...
        res.json({
            status: 'success',
            data: { eliminado: servicio[0], mensaje: `Servicio "${servicio[0].nombre}" eliminado correctamente` }
        });
    } catch (error) {
